...
# 将数据框导出为XLSX文件
# write_xlsx(strwb_survey_chem, path = "strwb_survey_chem.xlsx")
# write_xlsx(strwb_survey_mkt, path = "strwb_survey_mkt.xlsx")
# write_xlsx(strwb_census, path = "strwb_census.xlsx")
# write_xlsx(strwb_survey, path = "strwb_survey.xlsx")
```
...
lineplot1 <- ggplot(proportion_data, aes(x = Year, y = proportion, color = State)) +
  geom_line() +
...
lineplot2 <- ggplot(proportion_data, aes(x = Year, y = proportion, color = State)) +
  geom_line() +
  geom_point(aes(label = sprintf("%.3f", proportion)), size = 3) +
  labs(
    x = "Year",
...
lineplot3 <- ggplot(proportion_data, aes(x = Year, y = proportion, color = State)) +
  geom_line() +
  geom_point(aes(label = sprintf("%.3f", proportion)), size = 3) +
  labs(
    x = "Year",
...
print("In R, I have a data frame called 'strwb_survey_chem_CAS' with columns 'Year' 'State' 'temp23' 'chemical_name' 'CAS_number' and 'Value'. I want to create a line plot using ggplot2 to visualize the proportion of 'FUNGICIDE' usage over time (from 2016 to 2018 to 2019 to 2021) for four different states (CALIFORNIA, FLORIDA, OREGON, WASHINGTON). The proportion is defined as the count of 'FUNGICIDE' divided by the sum of the counts of the four types of chemicals: 'FUNGICIDE', 'HERBICIDE', 'INSECTICIDE' and 'OTHER'. The plot should have four lines of different colors, each representing a different state, a legend, x-label as 'state',  y-label as 'the proportion',  and a plot title as 'The Proportion of the Utility of Fungicide', Additionally, the points on each line should display the count of 'FUNGICIDE' divided by the sum of the count of the four types of chemicals with three decimal places.")

print("'OREGON' and 'WASHINGTON' have data for 2016 but not for other years, it is likely due to a lack of data availability for these states in the subsequent years (2018, 2019, and 2021) in the dataset. In other words, the dataset we are working with might not include information for those states in those specific years, resulting in no data points for those combinations.")
```
...
```{r}
cas <- unique(strwb_survey_chem_CAS$PCcodes)
as.vector(cas)
print("------------------------------------------------------------------------------------------------------------------------------------")
PCcodes <- cas[nchar(cas) == 6 & grepl("^[0-9]+$", cas)]
PCcodes <- as.data.frame(PCcodes)
PCcodes$cas_valid <- c("131860-33-8", NA, "188425-85-6", "180409-60-3", "121552-61-2", "119446-68-3", "907204-31-3", "39148-24-8", NA, "70630-17-0", "88671-89-0", "146659-78-1", "60207-90-1", "53112-28-0", "112281-77-3", "23564-05-8", "141517-21-7", "68694-11-1", "128639-02-1", "103361-09-7", "38641-94-0", "70901-12-1", "15299-99-7", "42874-03-3", "40487-42-1", "71751-41-2", "108168-76-9", NA, "82657-04-3", "69327-76-0", "400882-07-7", "153233-91-1", "13356-08-6", "39515-41-8", "134098-61-6", "158062-67-0", NA, NA, "78587-05-0", "138261-41-3", "161050-58-4", "116714-46-6", "96489-71-3", "95737-68-1", NA, "131929-60-7", "334-48-5", "124-07-2", "76674-21-0", "8000-78-0", NA, "99129-21-2", "87674-68-8", "81406-37-3", "100784-20-1", "145701-23-1", "525-79-1", NA, "67892-31-3", "77182-82-2", "122836-35-5", "137497-61-1", NA, NA, "66332-96-5", "52315-07-8", "36734-19-7", "155569-91-8", "91465-08-6", "203313-25-1", "57754-85-5", "15708-41-5", "81777-89-1")
PCcodes <- PCcodes |>
  drop_na()
# View(PCcodes) 
...
```{r}
library(dplyr)
# strwb_survey_chem_with_valid_CAS <- strwb_survey_chem_CAS
# strwb_survey_chem_with_valid_CAS <- merge(strwb_survey_chem_with_valid_CAS, PCcode, by.x = "PCcodes", by.y = "PCcode", all.x = TRUE)
# strwb_survey_chem_with_valid_CAS$CAS_number <- strwb_survey_chem_with_valid_CAS$cas_valid
# strwb_survey_chem_with_valid_CAS$cas_valid <- NULL
# View(strwb_survey_chem_with_valid_CAS)
